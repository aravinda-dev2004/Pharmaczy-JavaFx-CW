    ...
    public void deleteSupplier(Integer id) {
        try {
            ...
            PreparedStatement deleteSalesDetails = connection.prepareStatement(
                    "DELETE FROM sales_details WHERE batch_id IN (SELECT BatchID FROM batches WHERE SupplierID = ?)"
            );
            ...
            PreparedStatement deleteBatches = connection.prepareStatement(
                    "DELETE FROM batches WHERE SupplierID = ?"
            );
            deleteBatches.setObject(1, id);
            deleteBatches.execute();


            PreparedStatement deleteSupplier = connection.prepareStatement(
                    "DELETE FROM suppliers WHERE SupplierID = ?"
            );
            deleteSupplier.setObject(1, id);
